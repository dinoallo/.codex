...
variable "pm_snippets_storage" {
  description = "Proxmox storage containing cloud-init snippets referenced by cicustom (must support content type 'snippets')."
  type        = string
  default     = "local"
}
...
variable "vm_name_prefix" {
  description = "Prefix to use for VM names and hostnames (e.g. 'server'). A number will be appended: <prefix>-1"
  type        = string
  default     = "server"
}
...
variable "artifacts_dir" {
  description = "Per-stack output directory for generated key, snippets, and inventory."
  type        = string
  default     = ".artifacts"
}
